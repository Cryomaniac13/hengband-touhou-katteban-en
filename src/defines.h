...
// Shift day/dungeon one row down in translation because hunger
// status got shifted down as well
#ifdef JP
//#define ROW_DAY                 21
#define ROW_DAY                 24
...
// State won't fit on the same line as hunger status
// in translation, shift it one row down
#ifdef JP
//#define ROW_STATE               20
#define ROW_STATE               23
...
{\
	u32b tmp;\
	if(!start_time)msg_format(_("ERROR:start_timeが未設定のままupdate_playtime()が呼ばれた", "ERROR:update_playtime() called with start_time unset"));\
	tmp = time(NULL);\
