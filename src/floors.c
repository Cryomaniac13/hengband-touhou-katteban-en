...
void change_floor(void)
{
	...
	else
	{
		...
		if (loaded)
		{
			...
			for (i = 1; i < m_max; i++)
			{
				...
				{
					if(cheat_hear)
						msg_format(_("ユニーク(ID:%d)が別階層での再登場によりこの階から消えた",
                                    "Unique monster (ID: %d) appeared on different floor and was removed"),m_ptr->r_idx);
					/* Disapper from here */
					delete_monster_idx(i);
	...
	if(p_ptr->pclass == CLASS_SEIJA && flag_seija_quest_comp)
	{
		...
		msg_print(_("あなたは頼み事を聞いて感謝されてしまった！力が抜ける・・",
                    "Oh no! You received gratitude for completing a quest! You feel less powerful..."));
...
//		msg_print("床上のアイテムが呪文を跳ね返した。");
	...
	if (up)
	{
				msg_print(_("天井から伸びる岩を階段状に削った。",
                            "Stone extends from the ceiling, forming a staircase."));
		cave_set_feat(py, px,
	...
	{
				msg_print(_("地面に螺旋状の穴を開けた。",
                            "A spiraling staircase opens up in the ground."));
		cave_set_feat(py, px,
			(dest_sf_ptr->last_visit && (dest_sf_ptr->dun_level >= dun_level + 2)) ?
