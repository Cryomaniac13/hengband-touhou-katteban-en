...
//		         && !((q_idx == QUEST_OBERON) || (q_idx == QUEST_SERPENT))))
		...
		{
			sprintf(note_level_buf, _("エクストラクエスト(%d階):", "Extra quest (L%d):"), dun_level);
	...
	switch(type)
	{
		...
		case NIKKI_ARENA:
		{
			...
			if (num < 0)
			{
				fprintf(fff, _(" %2d:%02d %20s 「%s」の夢の住人たちに敗れ去った。\n",
                                " %2d:%02d %20s lost to dream residents of '%s'.\n"), hour, min, note_level, nightmare_stage_table[nightmare_rank].desc);
			}
			//勝利
			else
			{
				fprintf(fff, _(" %2d:%02d %20s 「%s」の夢の住人たちに勝利した。\n",
                                " %2d:%02d %20s defeated dream residents of '%s'.\n"), hour, min, note_level, nightmare_stage_table[nightmare_rank].desc);
			}
...
#else
					fprintf(fff, "%s was buried under falling rocks and ran away.\n", note);
#endif
					break;
...
static void do_cmd_options_cheat(cptr info)
{
	...
	while (TRUE)
	{
		...
		if(p_ptr->pseikaku == SEIKAKU_COLLECTOR)
		{
			...
			prt("                   <<  Warning  >>", 11, 0);
			prt("     If you turn on cheat/debug options,", 12, 0);
			prt("     items at your house entrance won't carry over to next Collector.", 13, 0);
		...
		else
		{
			/* 詐欺オプションをうっかりいじってしまう人がいるようなので注意 */
			prt("                               <<  Warning  >>", 11, 0);
			prt("   If you turn on cheat/debug options, you won't be able to register your score!", 12, 0);
			prt("   Score won't be recorded even if you turn it off later. If you're aiming to", 13, 0);
			prt("   become a ***WINNER***, please don't touch this.", 14, 0);
...
void do_cmd_version(void)
{
	...
	msg_format("変愚蛮怒勝手版 %d.%d.%dT",
		    H_VER_MAJOR, H_VER_MINOR, H_VER_MINOR);
#else
	msg_format("You are playing Hengband Katteban %d.%d.%dT",
		    H_VER_MAJOR, H_VER_MINOR, H_VER_MINOR);
#endif
}
...
void do_cmd_feeling(void)
{
	...
	{
		msg_print(_("典型的なクエストのダンジョンのようだ。",
...
#else
			msg_print("Looks like strange wilderness.");
#endif
...
///del131211 コンバットmsg
	...
	for (j = 1; j < m_max; j++)
	{
		...
		if(r_ptr->flags3 & RF3_WANTED || r_ptr->flags2 & RF2_TROPHY) msg_print(_("この階にお尋ね者がいるような気がする...",
                                                                                "You have a feeling that a wanted person is on this level..."));
	}
...
static cptr monster_group_text[] =
{
	...
	"Wight/Wraith/etc",
	"Xorn/Xaren/etc",
	"Greater youkai/Humanoid youkai",
	"Zephyr Hound",
	"Mimic",
	"Wall/Plant/Gas",
	"Mushroom patch",
...
static void do_cmd_knowledge_artifacts(void)
{
	...
	{
	    msg_format(_("そんなものはない。", "Those don't exist."));
		return;
	}
...
#else
		fprintf(fff, "      Surface  undefeated: %3d\n", n_alive_surface);
#endif
...
#else
		fprintf(fff, "Level %3d-%3d  undefeated: %3d\n", 1 + i * 10, 10 + i * 10, n_alive[i]);
#endif
...
#else
		fprintf(fff, "Level 101-     undefeated: %3d\n", n_alive_over100);
#endif
...
#else
		fputs("-------------  ----------\n", fff);
		fprintf(fff, "        Total  undefeated: %3d\n\n", n_alive_total);
#endif
	}
	else
	{
...
#else
		fputs("No known undefeated uniques.\n", fff);
#endif
	}
...
#else
	show_file(TRUE, file_name, "Undefeated Uniques", 0, 0);
#endif
...
static void do_cmd_knowledge_spell_exp(void)
{
	...
	{
	    msg_format(_("あなたは魔法書を使えない。", "You cannot use magic books."));
	    return;
	}
...
#else
        if(cp_ptr->skill_aptitude[i] == 0) fprintf(fff, "(n/a)");
#endif
...
#else
	show_file(TRUE, file_name, "Skill Proficiency", 0, 0);
#endif
...
#else
	fprintf(fff, "   Total: %d follower%s.\n",
...
#else
	show_file(TRUE, file_name, "Current Followers", 0, 0);
#endif
...
static void do_cmd_knowledge_buildings(void)
{
	...
	if (inven_special[INVEN_SPECIAL_SLOT_KOGASA_SMITH].k_idx)
	{
		...
		if (need_turn > 0)
		{
			...
			fprintf(fff, _("小傘に%sを預けている。\n約束の刻限まであと%02d:%02dだ。\n\n",
                            "Gave %s to Kogasa.\nTime left: %02d:%02d"),o_name,h,m);

		}
		else
		{
			fprintf(fff, _("小傘に%sを預けている。もう作業が済んでいるはずだ。\n\n",
                            "Gave %s to Kogasa. She's already finished.\n\n"), o_name);
		}
	...
	if (p_ptr->grassroots > 1)
	{
		...
		if (need_turn > 0)
		{
			...
			fprintf(fff, _("草の根妖怪ネットワークにアイテムを預けている。\nアイテム交換会の開始まであと%02d:%02dだ。\n\n",
                            "Gave an item to Grassroots Network youkai.\nExchange starts in: %02d:%02d."), h, m);
		}
		else
		{
			fprintf(fff, _("草の根妖怪ネットワークでアイテム交換会が始まっている頃だ。神社に急ごう。\n\n",
                            "Grassroots Network youkai started item exchange meeting. Hurry to the shrine.\n\n"));
		}
		flag_notice_sth = TRUE;
	}
	...
	else if (p_ptr->today_mon)
	{
...
#else
            fprintf(fff, "Reimu's divination says you'll get a treasure if you visit level %d of %s today.\n\n",target_level, (d_name + d_info[target_dungeon].name));
#endif
			flag_notice_sth = TRUE;
			break;
		...
		case FORTUNETELLER_TROPHY:
			...
			fprintf(fff, _("霊夢の占いによると、今日中に%sを倒すと幸福が訪れるらしい。\n\n",
                            "Reimu's divination says you'll be in luck if you defeat %s today.\n\n"), (r_name + r_info[target_r_idx].name));
			flag_notice_sth = TRUE;
			break;
		case FORTUNETELLER_GOODLUCK:
			fprintf(fff, _("霊夢の占いによると、今日の自分の運気は絶好調らしい！\n\n",
                            "Reimu's divination says that today is your lucky day!\n\n"));
			flag_notice_sth = TRUE;
			break;
		case FORTUNETELLER_BADLUCK:
			fprintf(fff, _("霊夢の占いによると、今日の自分の運気は絶不調らしい……\n\n",
                            "Reimu's divination says you're out of luck today......"));
			flag_notice_sth = TRUE;
			break;
		default:
			fprintf(fff, _("(ERROR:この占い結果のメッセージが定義されていない)\n\n",
                            "(ERROR: No message found for divination result)\n\n"));
	...
	if (inven_special[INVEN_SPECIAL_SLOT_ORIN_DELIVER].k_idx)
	{
		...
		if (need_turn > 0)
		{
			...
			fprintf(fff, _("火焔猫燐に%sを配達してもらっている。\nあと%02d:%02dくらいで着くはずだ。\n\n",
                            "Rin Kaenbyou is delivering %s.\nIt'll arrive in %02d:%02d.\n\n"), o_name, h, m);

		}
		else
		{
			fprintf(fff, _("火焔猫燐に%sを配達してもらっている。もう到着しているはずだ。\n\n",
                            "Rin Kaenbyou is delivering %s. It should have arrived by now.\n\n"), o_name);
		}
	...
	if(!flag_notice_sth)
		fprintf(fff, _("特に心に留めておくことはない。\n\n", "You don't have anything particular to keep in mind.\n\n"));

	my_fclose(fff);
	show_file(TRUE, file_name, _("現在の施設利用状況", "Current building services status"), 0, 0);
...
#else
	fprintf(fff,"   Total: %lu creature%s defeated.\n",
...
#else
	show_file(TRUE, file_name, "Defeated Enemies", 0, 0);
#endif
...
static void do_cmd_knowledge_monsters(bool *need_redraw, bool visual_only, int direct_r_idx)
{
	...
	while (!flag)
	{
		...
		if (redraw)
		{
			...
			if (p_ptr->wizard || visual_only) prt("Idx", 4, 62);
			prt("Sym", 4, 68);
			if (!visual_only) prt("Defeated", 4, 73);
#endif
...
static void do_cmd_knowledge_stat(void)
{
	...
	if (fff)
	{
		...
		*/
...
#else
		if (p_ptr->knowledge & KNOW_HPRATE) fprintf(fff, "Current Life Rating : %d/100\n", percent);
		else fprintf(fff, "Current Life Rating : ???\n");
		//		fprintf(fff, "能力の最大値\n\n");
		...
		extra_mode_score_mult(&mult_race_class, p_ptr->pclass);
		fprintf(fff, "Your score rating : %d%%\n", mult_race_class);
		...
		if (p_ptr->prace == RACE_ZASHIKIWARASHI)
		{
			...
			object_desc(o_name, o_ptr, OD_NAME_AND_ENCHANT);
			fprintf(fff, "Your mobile household : %s\n",o_name);
...
#else
	show_file(TRUE, file_name, "About yourself", 0, 0);
#endif
			...
			if (quest[i].type != QUEST_TYPE_RANDOM)
			{
				...
				if (quest[i].status == QUEST_STATUS_TAKEN || quest[i].status == QUEST_STATUS_STAGE_COMPLETED)
				{
					...
					case QUEST_TYPE_KILL_ANY_LEVEL:
...
#else
							plural_aux(name);
							sprintf(note," - defeat %d %s, have defeated %d.",
						...
						else if (i == QUEST_DREAMDWELLER)
						{
							if(is_special_seikaku(SEIKAKU_SPECIAL_SUMIREKO))
								sprintf(note, _(" - あなたの悪夢を倒す。", " - defeat your nightmare form."));
							else
								sprintf(note, _(" - 夢の世界のあなたを倒す。", " - defeat your Dream World form."));
						}
						else
#ifdef JP
							sprintf(note," - %sを倒す。",name);
#else
							sprintf(note," - defeat %s.",name);
#endif
						break;
						//通常アイテム収集　今のところ魔理沙専用
					case QUEST_TYPE_FIND_OBJECT:
						sprintf(note,_(" - 面白そうな本を見つけ出す。", " - look for interesting books."));
						break;
					...
					case QUEST_TYPE_FIND_ARTIFACT:
						...
						sprintf(note,_(" - %sを見つけ出す。", " - locate %s."), name);
						break;
						//宝物庫が★なしのとき
					case QUEST_TYPE_FIND_RANDOM_ARTIFACT:
						if (quest[i].k_idx)
						{
							sprintf(note,_(" - 伝説の%sを見つけ出す。", " - locate a legendary %s."), (k_name + k_info[quest[i].k_idx].name));
						}
						break;
					...
					case QUEST_TYPE_DELIVER_ITEM:
						...
						if(i == QUEST_KAGUYA)
						{
							...
							if(kaguya_quest_level < KAGUYA_QUEST_MAX)
								sprintf(note,format(_(" - %sを探し出す。",
                                                    " - find %s."), a_name+a_info[kaguya_quest_art_list[kaguya_quest_level]].name));
							else
								sprintf(note,format(_(" - %sを探し出す。",
                                                    " - find %s."),k_name+k_info[lookup_kind(TV_SOUVENIR,SV_SOUVENIR_KINKAKUJI)].name));
						}
						else
							sprintf(note,_(" - 指定されたアイテムを届ける。", " - deliver the specified item."));

						break;
					case QUEST_TYPE_BOUNTY_HUNT:
						r_ptr = &r_info[quest[i].r_idx];
						strcpy(name, r_name + r_ptr->name);
						sprintf(note,_(" - ダンジョンに潜伏したお尋ね者を倒す。",
                                        " - search the dungeon and defeat the wanted person."));
						break;
...
#else
						sprintf(note," - defeat %d monsters, have defeated %d.",
...
#else
						sprintf(note," - defeat all monsters.");
#endif
						break;
					}
				}
				...
				if (quest[i].status == QUEST_STATUS_COMPLETED)
				{
					...
					if(i == QUEST_MOON_VAULT || i == QUEST_MARICON)
						sprintf(tmp_str, _("    クエスト達成 - まだ脱出していない。\n",
                                            "    Quest complete - still have to escape.\n"));
					else
						sprintf(tmp_str, _("    クエスト達成 - まだ報酬を受けとってない。\n",
                                            "    Quest complete - still have to receive reward.\n"));
...
void do_cmd_knowledge(void)
{
	...
	while (1)
	{
		...
		if (p == 0)
		{
			...
			prt("(4) Display known monsters", 9, 5);
			prt("(5) Display defeated enemies", 10, 5);
			//if (!vanilla_town) prt("(6) Display wanted monsters", 11, 5);
			prt("(6) Display building services status", 11, 5);
			prt("(7) Display current followers", 12, 5);
		...
		else
		{
			...
			prt("(e) Display skill proficiency", 10, 5);
